...
def make_per_phantom_panels(phantom, df_merged, df_cands, df_proxy, old_df_sar, old_summary):
    ...
    ax.set_title(
        f"B  Proxy vs actual SAR – {label}\nSpearman ρ = {rho:.2f}  (p = {pval:.3f},  n = {len(df_merged)})",
    ...
    if old_df_sar is not None:
        ...
        ax.axhline(old_worst, color="orange", lw=2, ls="--",
                   label=f"old config worst ({old_worst:.4f} mW/kg)")
        ax.legend(fontsize=9)
    ...
    if old_summary:
        ...
        ax.hist(old_zs, bins=20, color="orange", alpha=0.6,
                label=f"old config (n={len(old_zs)})")
    ax.set_xlabel("Focus voxel Z (height index)", fontsize=11)
    ax.set_ylabel("# candidates", fontsize=11)
    ax.set_title(f"E  Body-height coverage: old vs new config – {label}",
                 fontsize=12, fontweight="bold")
    ...
    ax.set_title(f"G  Proxy vs whole-body SAR (ρ = {rho_wb:.2f}) – {label}",
                 fontsize=12, fontweight="bold")
    ...
    if old_df_sar is not None:
        old_worst = old_df_sar["peak_sar_mW_kg"].max()
        lines += ["", f"OLD worst:  {old_worst:.4f} mW/kg",
                  f"New / Old  = {worst_sar / old_worst:.2f}×"]
...
def make_combined_panels(all_stats):
    ...
    for s in all_stats:
        ...
        ax.scatter(dm["proxy_score"], dm["peak_sar_mW_kg"],
                   ...
                   s=60, alpha=0.75, label=f"{s['phantom'].capitalize()} (ρ={s['rho']:.2f})",
                   zorder=3)
    ...
    fig.suptitle("Z3  Ranked SAR for all phantoms – 700 MHz, new config",
    ...
    fig.suptitle("Z4  Correlation & coverage – all phantoms",
                 fontsize=13, fontweight="bold")
    fig.tight_layout()
    ...
    lines = ["SUMMARY – all phantoms, 700 MHz, new config", "=" * len(header), header,
             "-" * len(header)]
    for s in all_stats:
        lines.append(
