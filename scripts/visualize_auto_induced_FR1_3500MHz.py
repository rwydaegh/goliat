...
def make_per_phantom_panels(phantom, df_merged, df_cands, df_proxy):
    ...
    ax.set_title(f"G  Proxy vs whole-body SAR (rho = {rho_wb:.2f}) - {label}", fontsize=12, fontweight="bold")
...
def make_combined_panels(all_stats):
    ...
    ax.set_title(f"Z1  Worst-case SAR comparison across phantoms\n{FREQ_LABEL}, new config, 1 mW input", fontsize=12, fontweight="bold")
    ...
    for s in all_stats:
        ...
        ax.scatter(
            ...
            s=60,
            alpha=0.75,
            label=f"{s['phantom'].capitalize()} (rho={s['rho']:.2f})",
            zorder=3,
        )
    ...
    fig.suptitle(f"Z3  Ranked SAR for all phantoms - {FREQ_LABEL}, new config", fontsize=13, fontweight="bold")
    fig.tight_layout()
    ...
    fig.suptitle(f"Z4  Correlation & coverage - all phantoms ({FREQ_LABEL})", fontsize=13, fontweight="bold")
    fig.tight_layout()
    ...
    ax.set_ylabel("Peak SAR$_{10g}$ [mW/kg]", fontsize=11)
    ax.set_title(
        f"Z5  SAR peak body height across all phantoms ({FREQ_LABEL})\n(* = worst-case per phantom)", fontsize=12, fontweight="bold"
    )
    ax.legend(fontsize=10)
    ...
    lines = [f"SUMMARY - all phantoms, {FREQ_LABEL}, new config", "=" * len(header), header, "-" * len(header)]
    for s in all_stats:
        lines.append(
