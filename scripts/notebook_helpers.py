...
def run_bash(command, cwd=None):
    ...
    for possible_bash in ["bash", "C:\\Program Files\\Git\\bin\\bash.exe", "C:\\Program Files\\Git\\usr\\bin\\bash.exe"]:
        try:
            result = subprocess.run([possible_bash, "--version"], capture_output=True, timeout=2)
    ...
    try:
        ...
        process = subprocess.Popen([bash_exe, "-c", full_command], cwd=str(project_root), **popen_kwargs)
