...
Output: paper/far_field_NEW/figures/sar_trends_fr1_to_fr2.{png,pdf}
"""
...
OUT_DIR = REPO / "paper" / "far_field_NEW" / "figures"
...
def load_fr1_12diravg_wb():
    ...
    p = FR1_BASE / "results" / "far_field" / "thelonious" / "line" / "sar_line_environmental.csv"
    # Fallback path inside plots directory
    p2 = REPO / "plots" / "far_field" / "thelonious" / "line" / "sar_line_environmental.csv"
    if p2.exists():
        df = pd.read_csv(p2)
    else:
        raise FileNotFoundError(f"Cannot find sar_line_environmental.csv; tried {p2}")
        ...
        records.append(
            {
                ...
                "wb_mwkg":   d["whole_body_sar"] * SCALE_HALF * 1e3,
...
def make_plot(fr1_xneg, fr1_wb12, fr3):
    ...
    # ---- Skin SAR ----
    ax.plot(
        fr1_xneg["freq_ghz"], fr1_xneg["skin_mwkg"],
        color=C_SKIN, linestyle="solid", marker="o",
        markersize=MS_FR1, linewidth=LW, label="Skin SAR",
        markerfacecolor=C_SKIN,
    )
    ax.plot(
        fr3["freq_ghz"], fr3["skin_mwkg"],
        color=C_SKIN, linestyle="dashed", marker="o",
        markersize=MS_FR3, linewidth=LW, label="_nolegend_",
        markerfacecolor="white", markeredgecolor=C_SKIN,
    )

    # ---- Eye SAR ----
    ax.plot(
        fr1_xneg["freq_ghz"], fr1_xneg["eyes_mwkg"],
        color=C_EYES, linestyle="solid", marker="s",
        markersize=MS_FR1, linewidth=LW, label="Eye SAR",
        markerfacecolor=C_EYES,
    )
    ax.plot(
        fr3["freq_ghz"], fr3["eyes_mwkg"],
        color=C_EYES, linestyle="dashed", marker="s",
        markersize=MS_FR3, linewidth=LW, label="_nolegend_",
        markerfacecolor="white", markeredgecolor=C_EYES,
    )

    # ---- WB-SAR: FR1 = 12-dir avg (solid), FR3 = x_neg (dashed) ----
    ax.plot(
        fr1_wb12["freq_ghz"], fr1_wb12["wb_12dir_mwkg"],
        color=C_WB, linestyle="solid", marker="^",
        markersize=MS_FR1, linewidth=LW, label=r"SAR$_\mathrm{wb}$",
        markerfacecolor=C_WB,
    )
    ax.plot(
        fr3["freq_ghz"], fr3["wb_mwkg"],
        color=C_WB, linestyle="dashed", marker="^",
        markersize=MS_FR3, linewidth=LW, label="_nolegend_",
        markerfacecolor="white", markeredgecolor=C_WB,
    )
...
def main():
    ...
    print(f"  FR3 WB   @ 7.0 GHz:  {fr3.loc[fr3.freq_ghz==7.0, 'wb_mwkg'].values[0]:.2f}  (expect 7.92)")
