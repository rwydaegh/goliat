...
ax.axvline(
    proxy_df["proxy_score"].quantile(0.95),
    color="orange",
    linestyle="--",
    linewidth=2,
    label=f"95th pct: {proxy_df['proxy_score'].quantile(0.95):.3f}",
)
...
ax = axes[1, 1]
scatter = ax.scatter(proxy_df["z_mm"], proxy_df["proxy_score"], c=proxy_df["proxy_score"], cmap="viridis", alpha=0.4, s=2, rasterized=True)
...
print("Saved: fig1_proxy_distribution.png")
...
ax.scatter(
    corr_df.loc[max_idx, "proxy_score"], sapd_mW[max_idx], s=250, facecolors="none", edgecolors="red", linewidth=3, label="Worst case"
)
...
ax.set_title("(b) Rank Comparison\n(diagonal = perfect prediction)")
...
print("Saved: fig2_proxy_sapd_correlation.png")
