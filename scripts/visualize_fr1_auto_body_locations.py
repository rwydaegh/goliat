...
# ── constants ──────────────────────────────────────────────────────────────────
BASE = Path(__file__).resolve().parents[1] / "results" / "auto_induced_FR1" / "new_config"
...
for row_idx, freq in enumerate(["700MHz", "3500MHz"]):
    for col_idx, ph in enumerate(PHANTOMS):
        ...
        for lname, frac in LANDMARKS.items():
            ...
            if col_idx == 0:
                ax.text(0.44, y_mm, LNAME_SHORT[lname], va="bottom", ha="left", fontsize=5.5, color="gray", clip_on=True)
        ...
        if rows:
            ...
            for r in rows:
                ...
                if is_worst:
                    ...
                    ax.scatter(jx, r["focus_z_feet"], c=[fc], s=55, edgecolors="none", zorder=6, marker="D")
                else:
                    ax.scatter(jx, r["focus_z_feet"], c=[fc], s=14, edgecolors="k", linewidths=0.25, zorder=4, marker="o")

            # SAR peak location: star, colour-mapped, no edge
            worst_row = max(rows, key=lambda r: r["sar"])
            if worst_row["peak_z_feet"] is not None:
                ax.scatter(
                    0, worst_row["peak_z_feet"], c=[cmap(norm_color(worst_row["sar"]))], s=80, marker="*", edgecolors="none", zorder=7
                )
        ...
        title_str = f"{LABELS[ph]}\n{FREQ_TITLES[freq]}" if row_idx == 0 else FREQ_TITLES[freq]
        ax.set_title(title_str, fontsize=8, fontweight="bold", pad=2)

        ax.text(0.97, 0.97, PANEL_LABELS[row_idx][col_idx], transform=ax.transAxes, fontsize=6.5, fontweight="bold", va="top", ha="right")
...
legend_handles = [
    ...
    plt.Line2D(
        [0], [0], marker="D", color="w", markerfacecolor="gray", markersize=5.5, markeredgewidth=0, label="Worst-case focus location"
    ),
    plt.Line2D([0], [0], marker="*", color="w", markerfacecolor="gray", markersize=8, markeredgewidth=0, label="SAR peak location"),
]
