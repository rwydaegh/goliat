...
def upload_to_transfer_sh(zip_path: Path) -> str | None:
    ...
    try:
        ...
        with open(zip_path, "rb") as f:
            response = requests.put(f"https://transfer.sh/{zip_path.name}", data=f, headers={"Max-Days": "14"})
        ...
        try:
            result = subprocess.run(
                ["curl", "--upload-file", str(zip_path), f"https://transfer.sh/{zip_path.name}"], capture_output=True, text=True, check=True
            )
