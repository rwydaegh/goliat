...
class TimeDomainSimulation(object):
    ...
    def get_channel_flatenned_subclusters(self):
        ''' The channel is a 5D array with dimensions (N_rx, N_tx, N_clusters, N_subclusters, N_snapshots). 
        This function returns a 4D array with dimensions (N_rx, N_tx, N_clusters*N_subclusters, N_snapshots). 
        When there are no subclusters, the channel is zero. '''

        N_rx, N_tx, N_clusters, N_subclusters, N_snapshots = self.channel.shape
    ...
    def make_clusters_old(self, max_num_clusters=None, max_num_sub_clusters=None):
        ...
        for i_snapshot, num_sub_paths, lbs_pos, receiver_coll in tqdm(zip(self.snapshots, self.num_sub_paths.T, self.lbs_positions.T, self.receivers)):
    ...
    def make_clusters(self, prune_percentage):
        ...
        self.clusters = []
        for i_snapshot, rx_pos, receiver_coll in tqdm(zip(self.snapshots, self.rx_positions[:,self.rx,:].T, self.receivers)):
            cluster_collection = ClusterCollection()
        ...
        for grid, receiver, cluster_collection in zip(self.grids, self.receivers, self.clusters):
            if max_num_simulations is not None:
                if i>max_num_simulations:
                    break
            simulation =  DeterministicSimulation(grid, base_station=None, clusters=cluster_collection, receivers=receiver, polarization=self.polarization, frequency=self.frequency, scene=self.scene)
    ...
    def make_base_station_simulations(self, only_sides_with_thickness=None, max_num_simulations=None):
        ...
        for grid, receiver in zip(self.grids, self.receivers):
            if max_num_simulations is not None:
                if i>max_num_simulations:
                    break
            simulation = DeterministicSimulation(grid, base_station = self.base_station, receivers = receiver, polarization= self.polarization, frequency= self.frequency, scene = self.scene)
    ...
    def radiate_elements(self, snapshots = None, only_keep_hotspot_data = False, method = 'anywhere', **kwargs):
        ...
        for it in tqdm(range(len(snapshots))):
    ...
    def run_exposure_simulations_Sinc(self, snapshots = None, parallel = False, which_Sinc_to_compute = ['median', 'max', 'min', 'at_rx']):
        ...
        if parallel:
            ...
            max_Sinc_results = pool.map(process_simulation, [self.simulations[snap] for snap in snapshots])
            pool.close()
            pool.join()
    ...
    def plot_layout_all_snapshots(self, **kwargs):
        ...
        plt.figure('Layout')
        plt.ion()
        for i in tqdm(range(0,self.N_snapshots)):
            plt.clf()
    ...
    def hotspot_peak_analysis(self, method = 'anywhere', plot = True, verbose = True, **kwargs):
        ...
        if np.sum(non_finite_data) > 0:
            print('WARNING, non finite data found!')
        if np.sum(zero_data) > 0:
            print('WARNING, non finite data found!')
    ...
    def BioEM_figure_3(self):
        ...
        with open(path_of_this_file.parent+'/plot_data/path_changes.pickle', 'rb') as f:
        ...
        plt.tight_layout()
        plt.savefig(root / 'figures/Figure 3.png', bbox_inches='tight', dpi=300)
        plt.show()
