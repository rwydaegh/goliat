...
def _find_far_field_input_files(config, results_base_dir: Path, phantom: str, freq: int) -> list[Path]:
    """
        ...
        if len(time_diffs) > 3:
            ...
            if len(other_diffs) > 1:
                ...
                if max_diff > 2 * mean_diff:
                    ...
                    main_logger.warning(f"The largest time gap ({max_diff:.2f}s) is significantly larger than expected (mean: {mean_diff:.2f}s).")
...
def _find_near_field_input_files(config, results_base_dir: Path, phantom: str, freq: int) -> list[Path]:
    """
    ...
    for placement_name in enabled_placements:
        ...
        if len(found_files) > 1:
            main_logger.warning(f"{colorama.Fore.YELLOW}WARNING: Found {len(found_files)} input files in {results_folder}, expected 1. Using the most recent.")
