...
| Parameter | Type | Example Value | Description |
| :--- | :--- | :--- | :--- |
| `frequencies_mhz` | array | `[450, 700, 900]` | An array of frequencies in MHz to simulate. Each frequency generates a separate `.smash` project file. For multi-sine excitation (multiple frequencies in one simulation), use `"700+2450"` format, e.g., `["700+2450", 5800]`. |
...
| `power_balance.input_method` | string | `"bounding_box"` | Method for computing input power in far-field power balance. `"bounding_box"` uses simulation domain cross-section (default, gives ~100% balance). `"phantom_cross_section"` uses pre-computed phantom projected area (physically meaningful but gives >100% balance). |
