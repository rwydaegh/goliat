...
if __name__ == "__main__":
    ...
    print(f"Grid shape (nodes): {shape}")
    ...
    if args.chunk_test:
        ...
        for i, c in enumerate(comps):
            print(f"  comp{i}: shape={c.shape}, dtype={c.dtype}")
            print(f"          max|{args.field}|={np.abs(c).max():.4e}")
    else:
        ...
        for i, (idx, val) in enumerate(zip(test_indices, field_vals)):
            ...
            print(f"  [{idx[0]:3d},{idx[1]:3d},{idx[2]:3d}]: |{args.field}|={mag:.4e}")
