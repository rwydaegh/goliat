...
class SystemUtilizationPlot:
    ...
    def _refresh(self) -> None:
        ...
        lines = [line_cpu, line_ram]
        labels = [cpu_label, ram_label]

        # Plot GPU only if we have data
        if self.gpu_available and gpu_times:
            (line_gpu,) = self.ax.plot(gpu_times, gpu_values, "-", color="#ffd700", linewidth=1.0, label=gpu_label)  # type: ignore[arg-type]
            lines.append(line_gpu)
            labels.append(gpu_label)

        # Plot GPU VRAM only if we have data
        if self.gpu_available and gpu_vram_times:
            (line_vram,) = self.ax.plot(gpu_vram_times, gpu_vram_values, "-", color="#9d4edd", linewidth=1.0, label=gpu_vram_label)  # type: ignore[arg-type]
