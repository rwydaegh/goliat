...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parse verbose.log files to extract simulation metrics"
    )
    parser.add_argument(
        "log_path",
        type=str,
        help="Path to the verbose.log file"
    )
    parser.add_argument(
        "-o", "--output",
        type=str,
        default=None,
        help="Output path for JSON file (default: simulation_metrics.json in same directory)"
    )
    parser.add_argument(
        "--pretty",
        action="store_true",
        help="Print metrics to console in pretty format"
    )
