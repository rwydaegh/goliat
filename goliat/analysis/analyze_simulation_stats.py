...
def parse_all_logs(log_files: list[Path], verbose: bool = True) -> list[dict]:
    ...
    for i, log_file in enumerate(log_files):
        ...
        if verbose and i % 50 == 0:
            print(f"Parsing {i+1}/{len(log_files)}...")
    ...
    if verbose:
        print(f"Successfully parsed {len(all_metrics)} Near-Field log files.")
...
def compute_aggregate_stats(all_metrics: list[dict]) -> dict:
    ...
    stats = {
        ...
        "by_phantom": defaultdict(lambda: {
            "count": 0, "cells": 0, "iterations": 0, "cell_iterations": 0, "runtime_s": 0
        }),
        "by_frequency": defaultdict(lambda: {
            "count": 0, "cells": 0, "iterations": 0, "cell_iterations": 0, "runtime_s": 0
        }),
...
def create_visualizations(stats: dict, output_dir: str | Path):
    ...
    values = [stats["total_cell_iterations"], stats["total_edges"], 
              stats["total_cells_with_pml"], stats["total_iterations"]]
    ...
    bars = ax.barh(range(len(values)), values, color=colors[0], alpha=0.85)
    ...
    freq_list = sorted(stats["by_frequency"].keys(), 
    ...
    avg_speed_bcells_s = (stats["total_cell_iterations"] / stats["total_runtime_s"]) / 1e9 if stats["total_runtime_s"] > 0 else 0
    ...
    # Get frequency range
    freq_list_sorted = sorted(stats["by_frequency"].keys(), 
                              key=lambda x: int(re.search(r'\d+', x).group()) if re.search(r'\d+', x) else 0)
    if freq_list_sorted:
    ...
    ax_title.axis('off')
    ax_title.text(0.5, 0.5, "NEAR FIELD COMPUTATIONAL CAMPAIGN", 
                  ha='center', va='center', fontsize=30, color=colors[1], fontweight='bold')
    ...
    for i, (k, v, c) in enumerate(row2_stats):
        ...
        ax.axis('off')
        rect = plt.Rectangle((0.1, 0.15), 0.8, 0.7, transform=ax.transAxes, 
                             fill=True, color='#1e222b', zorder=0, alpha=0.8, lw=0)
        ax.add_patch(rect)
    ...
    ax_foot.axis('off')
    ax_foot.text(0.5, 0.5, f"Generated: {datetime.now().strftime('%Y-%m-%d')} | GOLIAT Solver Analysis", 
    ...
    if stats["cells_list"] and stats["runtime_list"]:
        ...
        sc = ax.scatter(cells_m, runtimes_min, c=colors[1], alpha=0.6, s=80, edgecolors='none')
        ...
        if len(cells_m) > 1:
            try:
                ...
                ax.text(0.05, 0.95, f"Correlation: {corr:.3f}", transform=ax.transAxes,
                        bbox=dict(boxstyle='round', facecolor='#1e222b', alpha=0.8), color='white')
            except:
                pass
...
def main():
    ...
    print("="*60)
    print("   GOLIAT NEAR-FIELD STATS ENGINE")
    print("="*60)
    ...
    if not all_metrics:
        print("No Near-Field logs found.")
        return
        
    stats = compute_aggregate_stats(all_metrics)
    
    print(f"\nTotal Near-Field Simulations: {stats['total_simulations']}")
    print(f"Total Cell-Iterations: {format_large_number(stats['total_cell_iterations'])}")
