...
def run_study_process(args):
    ...
    command = ["python", run_study_script, config_file, "--title", title, "--pid", str(process_id)]
...
def main():
    ...
    parser.add_argument('config', type=str, nargs='?', default="configs/near_field_config.json", help='Path or name of the configuration file (e.g., todays_far_field or configs/near_field_config.json).')
    parser.add_argument('--num-splits', type=int, default=4, help="Number of configs to split into (any positive integer that can be factored given the phantoms and frequencies/antennas available).")
