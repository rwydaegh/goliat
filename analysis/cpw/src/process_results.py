...
def plot_volume_vs_cpw(df, frequency_mhz):
    ...
    scatter = plt.scatter(plot_df['CPW'], plot_df['Total Volume'], c=plot_df['RelativePermittivity'], cmap='viridis', alpha=0.9, vmin=1)
    plt.yscale('log')
    plt.xlim(0, 25)
    ...
    for i, row in plot_df.iterrows():
        plt.annotate(row['Tissue'], (row['CPW'], row['Total Volume']), fontsize=8, textcoords="offset points", xytext=(0,5), ha='center')
...
def plot_cross_frequency(all_data, tissues, property_name):
    ...
    for tissue in tissues:
        ...
        if not tissue_data.empty:
            plt.plot(tissue_data['Frequency'], tissue_data[property_name], marker='o', label=tissue)
    ...
    if property_name == 'RequiredGridSize_mm':
        ...
        if not eye_data.empty:
            for i, row in eye_data.iterrows():
                plt.annotate(f"{row[property_name]:.2f}", (row['Frequency'], row[property_name]), textcoords="offset points", xytext=(0,5), ha='center')
...
def plot_required_gridding_per_freq(df, frequency_mhz):
    ...
    plt.figure(figsize=(14, 8))
    scatter = plt.scatter(plot_df['RequiredGridSize_mm'], plot_df['Total Volume'], c=plot_df['RelativePermittivity'], cmap='viridis', alpha=0.9, vmin=1)
    plt.xscale('log')
    plt.yscale('log')
    ...
    # Label all tissues
    for i, row in plot_df.iterrows():
        plt.annotate(row['Tissue'], (row['RequiredGridSize_mm'], row['Total Volume']), fontsize=8, textcoords="offset points", xytext=(0,5), ha='center')
...
def plot_aggregated_required_gridding(df):
    ...
    # Label all tissues
    for i, row in plot_df.iterrows():
        plt.annotate(f"{row['Tissue']} @ {int(row['Frequency'])}MHz", (row['RequiredGridSize_mm'], row['Total Volume']), fontsize=8, textcoords="offset points", xytext=(0,5), ha='center')
...
if __name__ == "__main__":
    ...
    if all_merged_data:
        ...
        critical_tissues = ['Brain (Grey Matter)', 'Skin', 'Muscle', 'Fat', 'Bone (Cortical)']
        ...
        plot_cross_frequency(combined_df, ['Eye_vitreous_humor', 'Stomach', 'Muscle'], 'RequiredGridSize_mm')
