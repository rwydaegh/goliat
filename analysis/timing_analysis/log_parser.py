...
def parse_log_file(log_file_path):
    ...
    frequency_blocks = re.split(r'--- Processing Frequency \d+/\d+: (\d+MHz) for Phantom \'(\w+)\' ---', log_content)
    ...
    for i in range(1, len(frequency_blocks), 3):
        ...
        base_setup_finalize_match = re.search(r'Total time: ([\d.]+) s\s+File: .*base_setup\.py\s+Function: BaseSetup\._finalize_setup', block)
        ...
        far_field_run_full_setup_match = re.search(r'Total time: ([\d.]+) s\s+File: .*far_field_setup\.py\s+Function: FarFieldSetup\.run_full_setup', block)
        ...
        far_field_create_entity_match = re.search(r'Total time: ([\d.]+) s\s+File: .*far_field_setup\.py\s+Function: FarFieldSetup\._create_simulation_entity', block)
        ...
        far_field_apply_common_match = re.search(r'Total time: ([\d.]+) s\s+File: .*far_field_setup\.py\s+Function: FarFieldSetup\._apply_common_settings', block)
        ...
        far_field_finalize_match = re.search(r'Total time: ([\d.]+) s\s+File: .*far_field_setup\.py\s+Function: FarFieldSetup\._finalize_setup', block)
