...
def parse_log_file(log_path):
    ...
    for block_content in frequency_blocks:
        ...
        freq_match = re.search(r"--- Processing Frequency (\d+/\d+): (\d+)MHz for Phantom '(\w+)' ---", block_content)
        if not freq_match:
            continue
        ...
        # Extract setup times
        setup_matches = re.finditer(r"Setting up simulation (\d+/\d+): (.*?)\n.*?Done in ([\d.]+)s", block_content, re.DOTALL)
        
        # Extract run times
        run_matches = re.finditer(r"--- Running simulation \d+/\d+: (EM_FDTD_.*?_(\d+MHz)_.*?) ---\n.*?Total simulation run time: ([\d.]+)s", block_content, re.DOTALL)
