...
from src.utils import Profiler as SimpleProfiler # Use the simple profiler for this analysis
import traceback
...
def analysis_process_wrapper(queue, frequency_mhz, sensitivity_config):
    ...
    try:
        ...
        class DummyGUI:
            def log(self, message, level='verbose'): pass
            def update_overall_progress(self, current_step, total_steps): pass
            def update_stage_progress(self, stage_name, current_step, total_steps): pass
        ...
        for i, value in enumerate(param_values):
            ...
            with profiler.subtask("config_creation"):
                ...
                temp_config_path = os.path.join(PROJECT_ROOT, sensitivity_config['config_dir'], f"{run_name}_config.json")
            ...
            with profiler.subtask("extract_results"):
                ...
                if study_type == 'near_field':
                    ...
                    orientation = list(config_data.get('placement_scenarios', {}).get(placement_scenario, {}).get('orientations', {}).keys())[0]
                ...
                for var_name, var_config in output_variables.items():
                    ...
                    queue.put({'type': 'status', 'message': f"  -> Extracted {var_name}: {val:.4f}" if val is not None else f"  -> WARNING: Could not extract {var_name}"})
            ...
            results_csv_path = os.path.join(PROJECT_ROOT, sensitivity_config['results_dir'], f'sensitivity_results_{frequency_mhz}MHz.csv')
            df.to_csv(results_csv_path, index=False)
            
            # Pickle results
            results_pickle_path = os.path.join(PROJECT_ROOT, sensitivity_config['results_dir'], f'sensitivity_results_{frequency_mhz}MHz.pkl')
        ...
        df = pd.DataFrame(results)
        if not df.empty:
            queue.put({'type': 'plot', 'df': df, 'freq': frequency_mhz, 'config': sensitivity_config})
...
def plot_results(df, frequency_mhz, config):
    ...
    for var in output_vars:
        ax.plot(df['param_value'], df[var], 'o-', label=f'{var.replace("_", " ").title()} @ {frequency_mhz} MHz')
...
def main():
    ...
    parser.add_argument('--frequency', type=int, default=700, help="The frequency in MHz to run the analysis for.")
