...
ax.plot(plot_data['grid_size'], plot_data['perforation'], 'o-', label='Perforation', markersize=4)

# Add labels to each point
for i, row in plot_data.iterrows():
    if row['perforation'] > 13.1:
        # Position text above the point
        ax.text(row['grid_size'], row['perforation'] * 1.2, f"{row['perforation']:.2f}\%", ha='center', va='bottom', fontsize=8)
    else:
        # Position text to the right of the point
        ax.text(row['grid_size'] + 0.15, row['perforation'], f"{row['perforation']:.2f}\%", ha='left', va='center', fontsize=8)
...
ax.text(zero_perforation_point / 2, 2, r'\textbf{No perforation}', rotation=90, verticalalignment='bottom', ha='center', fontsize=10)
